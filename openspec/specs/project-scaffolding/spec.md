...
The project SHALL provide an sbt-based Scala build that compiles cleanly and runs its test suite from a clean checkout without additional manual setup, using a pinned Scala version and JDK (Temurin 21).
...
- **THEN** the build resolves all declared dependencies and compiles with no errors

#### Scenario: Test suite runs green
- **GIVEN** a fresh clone of the repository
- **WHEN** a developer runs `sbt test`
- **THEN** the included smoke test executes and the suite reports success (exit code 0)

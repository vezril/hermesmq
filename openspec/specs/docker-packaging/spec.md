...
Define the container packaging for the service: a runnable, version-tagged Docker image built via sbt-native-packager that starts the process, runs as a non-root user, exposes a configurable port, serves the health endpoint, and stops cleanly on SIGTERM.

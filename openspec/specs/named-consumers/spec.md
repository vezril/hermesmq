# named-consumers Specification

## Purpose

Let consumers optionally name themselves on the consume operations so the broker can surface a per-subscription active-consumer count and attribute consume logs — observability only, without changing which messages are leased (a subscription is already a competing-consumer group).

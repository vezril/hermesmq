...
Define HermesMQ's two tracing roles: the edge (every inbound gRPC/REST request adopts or mints a correlation id, logs under it, and echoes it back) and the bus (a message's correlation id is adopted on publish, journaled durably, and delivered verbatim — the id other services correlate through across async hops).

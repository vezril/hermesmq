...
- **THEN** publish returns the assigned message id (with the dedup flag), pull returns the message with its ack id and payload, and ack completes successfully so a subsequent pull no longer returns it

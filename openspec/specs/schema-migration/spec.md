...
Apply the bundled PostgreSQL schema idempotently at startup — before the service serves — so a fresh database (k3s, compose, CI, or bare) provisions itself with no external init step, with a bounded wait for the database, fail-fast on error, and an opt-out for externally-managed migrations.

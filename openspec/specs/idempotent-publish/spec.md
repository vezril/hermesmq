# idempotent-publish Specification

## Purpose

Give producers an optional idempotency key so a retried publish is recognised and collapsed to the original within a bounded, per-topic window — providing exactly-once *publish* semantics as a safety net for at-least-once producers, without changing default behaviour.

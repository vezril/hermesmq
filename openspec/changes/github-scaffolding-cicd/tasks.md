...
- [ ] 4.1 Add `.github/workflows/ci.yml` triggering on `pull_request` and `push` to `development` and `main`
- [ ] 4.2 Configure steps: checkout, `actions/setup-java` (Temurin 21) with sbt dependency caching, `sbt compile`, `sbt test`
...
- [ ] 4.6 On `push` to `development`, add a publish step that publishes the dynver-derived snapshot package to GitHub Packages (owner from `github.repository_owner`, auth via `GITHUB_TOKEN`) after tests pass

## 5. Versioning & release workflow

- [ ] 5.1 Confirm `sbt-dynver` derives `X.Y.Z` on a `vX.Y.Z` tag and a pre-release/snapshot version off-tag (`sbt version` on a tagged vs untagged commit)
- [ ] 5.2 Add `.github/workflows/release.yml` triggering on tags matching `v*.*.*`
- [ ] 5.3 Configure release steps: checkout (full history for dynver), setup-java, run tests as a gate, `sbt publish` to GitHub Packages using `GITHUB_TOKEN`, then create a GitHub Release
...
- [ ] 5.6 Verify the malformed-tag edge case: a tag like `v1.2` / `release-1` does not trigger a release
...
- [ ] 6.4 Write root `README.md`: prerequisites (JDK 21, sbt), how to build (`sbt compile`), how to run tests (`sbt test`), the `main`/`development`/`feature/*` branch model, and the tag-driven SemVer release process
- [ ] 6.5 Document the required GitHub repository settings (branch protection, package registry) in the README so the setup is reproducible

## 7. Final verification

- [ ] 7.1 Fresh-clone check: from a clean clone, `sbt compile` and `sbt test` both succeed with no manual setup
- [ ] 7.2 Confirm all spec scenarios in `project-scaffolding` and `ci-cd-pipeline` have a corresponding verified task, then run `openspec validate` for the change

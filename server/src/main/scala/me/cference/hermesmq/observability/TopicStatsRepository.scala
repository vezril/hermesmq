...
import me.cference.hermesmq.domain.{TopicEvent, TopicId}
...
final class InMemoryTopicStatsRepository(using ExecutionContext) extends TopicStatsRepository with TopicStatsSink:
  ...
  def registerTopic(topicId: TopicId): Unit =
    state.updateAndGet(m => if m.contains(topicId) then m else m.updated(topicId, (0L, false)))
...
final class JdbcTopicStatsSink(conn: Connection) extends TopicStatsSink:
  ...
  def registerTopic(topicId: TopicId): Unit =
    ...
    ps.setString(1, topicId.value); ps.executeUpdate()
    ...
    Future {
      blocking {
        ...
        try
          ...
          val builder = List.newBuilder[TopicStats]
          while rs.next() do
            TopicId.from(rs.getString("topic_id")).toOption.foreach { id =>
              builder += TopicStats(id, rs.getLong("published_total"), rs.getBoolean("deleted"))
            }
          builder.result()
        finally conn.close()
      }
